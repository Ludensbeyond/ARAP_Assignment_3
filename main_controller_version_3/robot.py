        ...
        if self.camera_interval >= interval:
            ...
            for x in range(width):
                for y in range(height):
                    red += self.camera.imageGetRed(image, self.camera.getWidth(), x, y)
                    green += self.camera.imageGetGreen(image, self.camera.getWidth(), x, y)
                    blue += self.camera.imageGetBlue(image, self.camera.getWidth(), x, y)
                    pixel_count += 1
    
            average_red = red / pixel_count
            average_green = green / pixel_count
            average_blue = blue / pixel_count          
            ...
            if average_red > red_threshold:
                self.last_color_seen = "red"
            elif average_green > green_threshold:
                self.last_color_seen = "green"
            elif average_blue > blue_threshold:
                self.last_color_seen = "blue"
            else:
                self.last_color_seen = None
